...
python scripts\embed_ir.py ir\v1\ac\abc\296\C-gap-existence.json --update-existing embeddings\qwen3-embedding-0.6b-all
python scripts\embed_ir.py --update-existing embeddings\qwen3-embedding-0.6b-all --merge-from embeddings-patch\qwen3-embedding-0.6b-all
```
...
- `--update-existing` refreshes selected IR files inside an existing embedding
  output directory by replacing matching `embedding_id` rows in `embeddings.npy`
  and `documents.jsonl`; missing rows are appended.
- `--merge-from` with `--update-existing` merges a separately generated patch
  embedding directory into an existing full embedding directory without loading
...
python scripts\runpod_embed.py root@ssh.runpod.io --port 12345 --identity-file "$env:USERPROFILE\.ssh\id_ed25519" --model-size 4b --output-root embeddings-patch ir\v1\ac\abc\296\C-gap-existence.json
...
If optional IR paths are passed after the SSH target/options, only those files
or directories are passed to `scripts/embed_ir.py` on the pod. This is useful for
generating a small patch output root such as `embeddings-patch/`, then merging
it locally with `scripts/embed_ir.py --update-existing ... --merge-from ...`.
