...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output zip path. Defaults to data-bundle-YYMMDD-HHMMSS.zip in the repo root.",
    )
    return parser.parse_args(argv)


def zip_data_bundle(*, repo_root: Path, output: Path) -> Path:
    source_roots = [
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    output_path = zip_data_bundle(repo_root=repo_root, output=output)
